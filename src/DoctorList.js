...
function DoctorList() {
  ...
  const getAppointmentStatusKey = (appt) => {
    ...
    if (
      appt?.paymentStatus &&
      String(appt.paymentStatus).toUpperCase() === "PAID"
    )
      return "PAID";
    if (appt?.payment_method || appt?.paymentMethod || appt?.paidVia)
      return "PAID";
    try {
      const paidObj =
  ...
  const getBadgeClass = (appt) => {
    const key = getAppointmentStatusKey(appt);
    if (key === "PAID") return "bg-orange-500 text-white";
    if (key === "COMPLETED") return "bg-orange-100 text-orange-700";
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="w-64 bg-white text-gray-800 shadow-lg fixed left-0 top-0 h-full border-r border-gray-200 flex flex-col">
        ...
        {/* Version */}
        <div className="sidebar-support bg-center">
      ...
      <div className="ml-64 flex-1 p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          {/* Greeting Card */}
          <Card className="p-4 sm:p-6 bg-gradient-to-r from-orange-50 to-purple-50 shadow-lg border border-orange-200">
            <div className="flex items-center gap-3">
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-orange-400 to-purple-400 flex items-center justify-center text-white text-2xl font-bold">
                {user.initials}
              </div>
              <div>
          ...
          {/* Time & Shift Card */}
          <Card className="p-4 sm:p-6 bg-gradient-to-r from-orange-50 to-orange-50 shadow-lg border border-orange-200">
            <Typography
              variant="h5"
              className="mb-3 text-center font-bold text-orange-900"
            >
              Time & Shift
            </Typography>
        ...
        {/* Doctor List Section */}
        <Card className="p-4 sm:p-6 shadow-lg border border-orange-500 mb-6 bg-orange-50 hover:bg-orange-100 transition duration-200">
                    ...
                    >
                      ...
                      <div className="flex-1 w-full flex flex-col items-start text-left space-y-1 text-black">
                        ...
                        <div className="flex justify-between items-center w-full text-sm">
                          ...
                          <div className="flex items-center gap-1 bg-orange-100 px-2 py-1 rounded-full border border-orange-300">
                            {" "}
                          ...
                          <div className="w-full">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {doc.availableDays.map((day, i) => (
                                <span
                                  key={i}
                                  className="px-2 py-1 text-xs bg-orange-100 text-black-700 rounded-full border border-orange-300"
                                >
                                  {day}
                                </span>
                              ))}
                        ...
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <Switch
                            ...
                            onChange={(e) =>
                              handleToggle(realIndex, e.target.checked)
                            }
                            color="orange"
                          />
                        </div>
                        ...
                        {doc.phone && (
                          <Typography className="text-xs sm:text-sm flex items-center gap-1 break-all">
                            <MdOutlinePhoneIphone className="text-orange-600" />
                            <a
                              href={`tel:${doc.phone}`}
                        ...
                        {/* Appointments Card */}
                        {Array.isArray(doc.appointments) &&
                          doc.appointments.length > 0 && (
                            <div className="w-full mt-2 bg-gradient-to-r from-orange-50 to-indigo-50 border border-indigo-200 rounded-lg overflow-hidden">
                              ...
                              <div className="p-2">
                                {(() => {
                                  ...
                                  const totalPages = Math.max(
                                    1,
                                    Math.ceil(sorted.length / APPTS_PER_PAGE),
                                  );
                                            ...
                                            <div className="flex gap-1">
                                              {Array.from(
                                                ...
                                                <button
                                                  key={p}
                                                  onClick={() =>
                                                    setApptPage(realIndex, p)
                                                  }
                                                  className={`px-2 py-1 text-xs rounded ${p === page ? "bg-orange-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
                                                >
                                                  {p}
                                                </button>
                                              ))}
                      ...
                      <div className="mt-2 flex justify-center gap-2 flex-wrap">
                        <Button
                          size="sm"
                          className="flex items-center bg-orange-500 text-white hover:bg-orange-600 text-xs px-2 py-1"
                          onClick={() => {
                        ...
                        <Button
                          size="sm"
                          className="bg-orange-500 text-white hover:bg-orange-600 text-xs px-2 py-1"
                          onClick={() =>
                            setConfirmAction({
                              open: true,
                              type: "clone",
                  ...
                  (editDoctor.appointments || []).map((apt, ai) => (
                    ...
                    >
                      <div className="font-semibold text-sm">
                        {apt.patientName || apt.name || "Patient"}
                      </div>
                      <div className="text-xs text-gray-600">
                        Date: {apt.date || "-"}
                      </div>
                ...
                {editingAppointmentIndex !== null && (
                  <div className="mt-3 border-t pt-3">
                    ...
                    <input
                      ...
                      value={
                        tempAppointment.patientName ||
                        tempAppointment.name ||
                        ""
                      }
                      onChange={(e) =>
                        setTempAppointment({
                    ...
                    <input
                      ...
                      value={
                        tempAppointment.date
                          ? tempAppointment.date.split &&
                            tempAppointment.date.split("T")[0]
                          : tempAppointment.date || ""
                      }
                      onChange={(e) =>
                        setTempAppointment({
                    ...
                    <div className="mt-2 flex gap-2">
                      <Button
                        ...
                        onClick={() => {
                          ...
                          updated[editingAppointmentIndex] = {
                            ...updated[editingAppointmentIndex],
                            ...tempAppointment,
                          };
                          setEditDoctor({
