...
function MyCart() {
  ...
  const handleSubmit = () => {
    ...
    // ✅ ALSO add appointment to the specific doctor's record in doctors array
    const storedDoctors = JSON.parse(localStorage.getItem("doctors")) || [];
    const updatedDoctors = storedDoctors.map((doctor) => {
      const docFullName = `${doctor.firstName || ""} ${doctor.lastName || ""}`.trim();
      if (docFullName === savedAppointment.dentist) {
        return {
          ...doctor,
          appointments: [
            ...(doctor.appointments || []),
                {
                  patientName: savedAppointment.name || savedAppointment.customerName,
    ...
    // notify other components in same window to reload doctors
    try {
  ...
  const confirmPayment = (method) => {
    ...
    setTimeout(() => {
      ...
      console.debug("MyCart.confirmPayment: marking paid", { id: selectedAppointment.id, method });
      console.debug("MyCart.confirmPayment: before paidAppointments", paidAppointments);

      setPaidAppointments(updatedPaid);
      localStorage.setItem("paidAppointments", JSON.stringify(updatedPaid));
      console.debug("MyCart.confirmPayment: saved paidAppointments", updatedPaid);

      // ✅ Mark the appointment as paid in global appointment storage
      const storedAppointments = JSON.parse(localStorage.getItem("appointments")) || [];
      const updatedStoredAppointments = storedAppointments.map((appt) =>
        String(appt.id) === String(selectedAppointment.id)
          ? { ...appt, status: "Paid", paid: true, paymentMethod: method }
          : appt,
      );
      console.debug("MyCart.confirmPayment: updated appointments (matching)", updatedStoredAppointments.filter(a=>String(a.id)===String(selectedAppointment.id)));
      localStorage.setItem("appointments", JSON.stringify(updatedStoredAppointments));

      // ✅ Mark the appointment as paid on the doctor record too
      const storedDoctors = JSON.parse(localStorage.getItem("doctors")) || [];
      const updatedDoctors = storedDoctors.map((doctor) => ({
        ...doctor,
        appointments: (doctor.appointments || []).map((appt) =>
          (String(appt.appointmentID) === String(selectedAppointment.id) || String(appt.id) === String(selectedAppointment.id))
            ? { ...appt, status: "Paid", paid: true, paymentMethod: method }
            : appt,
        ),
      }));
      console.debug("MyCart.confirmPayment: updated doctors (matching appt entries)", updatedDoctors.map(d=>({name:`${d.firstName||''} ${d.lastName||''}`.trim(), matches: d.appointments?.filter(a=>String(a.appointmentID)===String(selectedAppointment.id)||String(a.id)===String(selectedAppointment.id))})).filter(d=>d.matches && d.matches.length>0));
      localStorage.setItem("doctors", JSON.stringify(updatedDoctors));
      try {
        window.dispatchEvent(new Event("doctorsUpdated"));
      } catch (e) {}

      // ✅ Also update appointment history if it exists
      const storedHistory = JSON.parse(localStorage.getItem("appointmentHistory")) || [];
      const updatedHistory = storedHistory.map((appt) =>
        String(appt.id) === String(selectedAppointment.id)
          ? { ...appt, status: "Paid", paid: true, paymentMethod: method }
          : appt,
      );
      console.debug("MyCart.confirmPayment: updated appointmentHistory (matching)", updatedHistory.filter(a=>String(a.id)===String(selectedAppointment.id)));
      localStorage.setItem("appointmentHistory", JSON.stringify(updatedHistory));
  ...
  appointments = appointments.map((a) =>
    String(a.id) === String(app.id) ? { ...a, status: "Cancelled" } : a
  );

  localStorage.setItem("appointments", JSON.stringify(appointments));

  console.debug("MyCart.handleCancel: updated appointments (matching)", appointments.filter(a=>String(a.id)===String(app.id)));

  // ✅ update doctor records as well
  const storedDoctors = JSON.parse(localStorage.getItem("doctors")) || [];
  const updatedDoctors = storedDoctors.map((doctor) => ({
    ...doctor,
    appointments: (doctor.appointments || []).map((a) =>
      (String(a.appointmentID) === String(app.id) || String(a.id) === String(app.id))
        ? { ...a, status: "Cancelled" }
        : a,
    ),
  }));
  console.debug("MyCart.handleCancel: updated doctors (matching appt entries)", updatedDoctors.map(d=>({name:`${d.firstName||''} ${d.lastName||''}`.trim(), matches: d.appointments?.filter(a=>String(a.appointmentID)===String(app.id)||String(a.id)===String(app.id))})).filter(d=>d.matches && d.matches.length>0));
  localStorage.setItem("doctors", JSON.stringify(updatedDoctors));
  try { window.dispatchEvent(new Event("doctorsUpdated")); } catch (e) {}
  // ✅ remove payment
