...
const CUSTOMER_TABS   = ["Dashboard", "New Ticket", "My Tickets", "FAQ", "Contact Channels", "Callback", "💬 Live Chat"];
const EXEC_TABS       = ["Dashboard", "New Ticket", "My Tickets", "FAQ", "Contact Channels", "Callback", "💬 Live Chat", "📞 Calls", "💬 Chats"];
const TEAM_LEAD_TABS  = ["Executives", "Dashboard", "New Ticket", "My Tickets", "FAQ", "Contact Channels", "Callback", "💬 Live Chat", "📞 Calls", "💬 Chats", "👥 Users"];
...
/* Default incentive policy — shared shape with Executive_Portal.js via the
   "incentivePolicy" localStorage key. The Team Lead edits it here; execs read it. */
const DEFAULT_INCENTIVE_POLICY = {
  enabled:           true,  // feature flag — Team Lead can turn incentives off
  currency:          "₹",
  perResolvedTicket: 25,
  perAnsweredCall:   10,
  perChatHandled:    15,
  answerRateTarget:  80,
  answerRateBonus:   200,
...
export default function ContactCenter() {
  ...
  /* Incentive policy (Team Lead editable) */
  ...
  const [incentiveDraft, setIncentiveDraft] = useState(null); // editable copy while editing
  const [incentiveSaved, setIncentiveSaved] = useState(false);

  const saveIncentivePolicy = (policy) => {
    setIncentivePolicy(policy);
    localStorage.setItem("incentivePolicy", JSON.stringify(policy));
    setIncentiveDraft(null);
    setIncentiveSaved(true);
    setTimeout(() => setIncentiveSaved(false), 2000);
  };

  // Feature flag toggle — updates the draft while editing, else persists immediately
  const toggleIncentiveEnabled = () => {
    if (incentiveDraft) {
      setIncentiveDraft(d => ({ ...d, enabled: !(d.enabled !== false) }));
    } else {
      const updated = { ...incentivePolicy, enabled: !(incentivePolicy.enabled !== false) };
      setIncentivePolicy(updated);
      localStorage.setItem("incentivePolicy", JSON.stringify(updated));
    }
  };
  ...
  const [execActiveCalls,  setExecActiveCalls]  = useState(() => {
    try { return JSON.parse(localStorage.getItem("execActiveCalls") || "{}"); } catch { return {}; }
  });
  const [bargeInSession,   setBargeInSession]   = useState(null); // active barge call object
  const [bargeMode,        setBargeMode]        = useState("listen"); // "listen" | "whisper" | "full"
  const [bargeTimer,       setBargeTimer]       = useState(0);
  const bargeTimerRef  = React.useRef(null);
  const bargeAudioRef  = React.useRef(null);
  const bargeMicRef    = React.useRef(null);

  /* ── Live Chat (customer side) ── */
  const [myChatId,     setMyChatId]     = useState(() => sessionStorage.getItem("myChatId") || null);
  const [chatSessions, setChatSessions] = useState(() => {
    try { return JSON.parse(localStorage.getItem("liveChatSessions") || "{}"); } catch { return {}; }
  });
  const [chatInput,    setChatInput]    = useState("");
  const [chatForm,     setChatForm]     = useState({ name: "", phone: "" });
  const chatEndRef = React.useRef(null);

  /* ── Live Chat (exec side) ── */
  const [execChatActiveId, setExecChatActiveId] = useState(null);
  const [execChatReply,    setExecChatReply]    = useState("");
  const execChatEndRef = React.useRef(null);
  ...
  const isTeamLead = execSession?.role === "Team Lead";
  const isExec     = !!execSession;
  const TABS       = isTeamLead ? TEAM_LEAD_TABS : isExec ? EXEC_TABS : CUSTOMER_TABS;

  /* if Team Lead logs out while on a restricted tab, fall back to Dashboard */
  useEffect(() => {
    if (!TABS.includes(activeTab)) setActiveTab(isTeamLead ? "Executives" : "Dashboard");
  ...
  useEffect(() => {
    const onStorage = (e) => {
      try {
        ...
        if (e.key === "execActiveCalls") {
          setExecActiveCalls(JSON.parse(e.newValue || "{}"));
        }
        if (e.key === "liveChatSessions") {
          setChatSessions(JSON.parse(e.newValue || "{}"));
        }
      } catch {}
    };
  ...
  useEffect(() => {
    const id = setInterval(() => {
      try { setOnlineSessions(JSON.parse(localStorage.getItem("execOnlineSessions") || "{}")); } catch {}
    }, 30000);
    return () => clearInterval(id);
  }, []);

  /* poll execActiveCalls every 5 s so barge-in button appears quickly */
  useEffect(() => {
    const id = setInterval(() => {
      try { setExecActiveCalls(JSON.parse(localStorage.getItem("execActiveCalls") || "{}")); } catch {}
    }, 5000);
    return () => clearInterval(id);
  }, []);

  /* poll liveChatSessions every 2 s for real-time chat messages */
  useEffect(() => {
    const id = setInterval(() => {
  ...
  }, [chatSessions, myChatId]);

  useEffect(() => {
    execChatEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [chatSessions, execChatActiveId]);

  /* barge-in timer */
  useEffect(() => {
    if (bargeInSession) {
      bargeTimerRef.current = setInterval(() => setBargeTimer(t => t + 1), 1000);
    } else {
      clearInterval(bargeTimerRef.current);
      setBargeTimer(0);
    }
    return () => clearInterval(bargeTimerRef.current);
  }, [bargeInSession]);

  /* barge-in simulated call audio */
  useEffect(() => {
    const stopAudio = () => {
      if (bargeAudioRef.current) {
        try { bargeAudioRef.current.noise.stop(); } catch {}
        try { bargeAudioRef.current.hum.stop(); } catch {}
        try { bargeAudioRef.current.ctx.close(); } catch {}
        bargeAudioRef.current = null;
      }
    };
    if (!bargeInSession) { stopAudio(); return; }

    try {
      const ctx = new (window.AudioContext || window.webkitAudioContext)();

      /* ambient white noise — simulates call background */
      ...
      noiseGain.gain.value = bargeMode === "full" ? 0.07 : 0.04;
      noise.connect(noiseGain);
      noiseGain.connect(ctx.destination);
      noise.start();

      /* subtle 50 Hz phone-line hum */
      const hum = ctx.createOscillator();
      hum.type  = "sine";
      hum.frequency.value = 50;
      const humGain = ctx.createGain();
      humGain.gain.value = bargeMode === "full" ? 0.02 : 0.008;
      hum.connect(humGain);
      humGain.connect(ctx.destination);
      hum.start();

      bargeAudioRef.current = { ctx, noise, hum, noiseGain, humGain };
    } catch {}

    return stopAudio;
  }, [bargeInSession, bargeMode]);

  /* mic activation for whisper / full barge modes */
  useEffect(() => {
    const stopMic = () => {
      if (bargeMicRef.current) {
        bargeMicRef.current.getTracks().forEach(t => t.stop());
        bargeMicRef.current = null;
      }
    };
    if (!bargeInSession || bargeMode === "listen") { stopMic(); return; }
    navigator.mediaDevices?.getUserMedia({ audio: true, video: false })
      .then(stream => { bargeMicRef.current = stream; })
      .catch(() => {});
    return stopMic;
  }, [bargeInSession, bargeMode]);
  ...
  /* ── Exec-side chat helpers ── */
  const acceptChatAsExec = (chatId) => {
    const all = JSON.parse(localStorage.getItem("liveChatSessions") || "{}");
    if (!all[chatId]) return;
    all[chatId] = { ...all[chatId], execId: execSession?.id, execName: execSession?.name, status: "active" };
    localStorage.setItem("liveChatSessions", JSON.stringify(all));
    setChatSessions({ ...all });
    setExecChatActiveId(chatId);
  };

  const sendExecChatMsg = () => {
    if (!execChatReply.trim() || !execChatActiveId) return;
    const all = JSON.parse(localStorage.getItem("liveChatSessions") || "{}");
    if (!all[execChatActiveId]) return;
    const msg = { id: Date.now(), sender: "exec", senderName: execSession?.name || "Support", text: execChatReply.trim(), ts: Date.now() };
    all[execChatActiveId] = { ...all[execChatActiveId], messages: [...all[execChatActiveId].messages, msg] };
    localStorage.setItem("liveChatSessions", JSON.stringify(all));
    setChatSessions({ ...all });
    setExecChatReply("");
  };

  const closeExecChatSession = (chatId) => {
    const all = JSON.parse(localStorage.getItem("liveChatSessions") || "{}");
    if (all[chatId]) all[chatId] = { ...all[chatId], status: "closed" };
    localStorage.setItem("liveChatSessions", JSON.stringify(all));
    setChatSessions({ ...all });
    if (execChatActiveId === chatId) setExecChatActiveId(null);
  };

  /* ── Live Chat helpers ── */
  const startChat = () => {
    const name  = chatForm.name.trim()  || customerSession?.name   || user.name  || "Customer";
    const phone = chatForm.phone.trim() || customerSession?.mobile || "";
    const id    = `chat_${Date.now()}`;
    const session = { id, customerName: name, customerPhone: phone, messages: [], execId: null, execName: null, status: "waiting", startedAt: Date.now() };
    const all = { ...(JSON.parse(localStorage.getItem("liveChatSessions") || "{}")), [id]: session };
    localStorage.setItem("liveChatSessions", JSON.stringify(all));
    sessionStorage.setItem("myChatId", id);
    setChatSessions(all);
    setMyChatId(id);
  };

  const sendChatMsg = () => {
    if (!chatInput.trim() || !myChatId) return;
    const all = JSON.parse(localStorage.getItem("liveChatSessions") || "{}");
    if (!all[myChatId]) return;
    const msg = { id: Date.now(), sender: "customer", senderName: all[myChatId].customerName, text: chatInput.trim(), ts: Date.now() };
    all[myChatId] = { ...all[myChatId], messages: [...all[myChatId].messages, msg] };
    localStorage.setItem("liveChatSessions", JSON.stringify(all));
    setChatSessions({ ...all });
    setChatInput("");
  };
  ...
  const endMyChat = () => {
    if (!myChatId) return;
    const all = JSON.parse(localStorage.getItem("liveChatSessions") || "{}");
    if (all[myChatId]) { all[myChatId] = { ...all[myChatId], status: "closed" }; }
    localStorage.setItem("liveChatSessions", JSON.stringify(all));
    setChatSessions({ ...all });
    sessionStorage.removeItem("myChatId");
    setMyChatId(null);
    setChatForm({ name: "", phone: "" });
  };
  ...
  const [transferTarget, setTransferTarget] = React.useState(null); // { call, source: "active" | "queue" }
  ...
  /* Transfer / assign a call to an executive — pushes to the shared
     "assignedCalls" queue that the Executive Portal polls & claims. */
  const assignCallToExec = (call, exec) => {
    const source = transferTarget?.source;
    const payload = {
      id: call.id || `TX-${Date.now()}`,
      name: call.name,
      number: call.number,
      direction: "inbound",
      assignedToId: exec.id,
      assignedToName: exec.name,
      fromLead: execSession?.name || "Team Lead",
      ts: Date.now(),
    };
    let list;
    try { list = JSON.parse(localStorage.getItem("assignedCalls") || "[]"); } catch { list = []; }
    if (!Array.isArray(list)) list = [];
    list.push(payload);
    localStorage.setItem("assignedCalls", JSON.stringify(list));

    if (source === "active") {
      hangUp("Transferred");
    } else {
      // remove from local inbound queue and log it as transferred
      setInboundQueue(q => q.filter(c => c.id !== call.id));
      setCallLog(prev => [{
        id: call.id, direction: "inbound", name: call.name, number: call.number,
        outcome: "Transferred", duration: 0, note: `Assigned to ${exec.name}`,
        assignedTo: exec.name, startedAt: call.arrivedAt, endedAt: new Date().toISOString(),
      }, ...prev]);
    }
    setTransferTarget(null);
    window.alert(`📞 Call ${source === "active" ? "transferred" : "assigned"} to ${exec.name}`);
  };
    ...
    <div className="flex min-h-screen" style={{ background: "#F5F5F5" }}>
      ...
      <aside className="w-64 bg-white fixed left-0 top-0 h-full border-r border-gray-200 flex flex-col z-20 shadow-sm">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
          {NAV_ITEMS.map(item => {
            const active = item.path === "/ContactCenter";
            return (
              <Link
      ...
      <main className="ml-64 flex-1 flex flex-col min-h-screen">
          ...
          return (
            <>
              <style>{`
                ...
                .incoming-card { animation: slideInRight 0.35s cubic-bezier(0.34,1.56,0.64,1); }
                .ring-btn      { animation: ringPulse 1.4s infinite; }
                .phone-icon    { animation: phoneWiggle 1s infinite; display:inline-block; }
              `}</style>
              <div className="incoming-card" style={{
                position: "fixed", top: 24, right: 24, zIndex: 80,
                ...
                {/* orange header */}
                <div style={{ background: "linear-gradient(135deg,#15803d,#16a34a,#22c55e)" }} className="px-5 pt-4 pb-3">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="phone-icon text-white text-lg">📞</span>
                    <span className="text-xs font-black text-orange-100 uppercase tracking-widest">Incoming Call</span>
                    {inboundQueue.length > 1 && (
                      <span className="ml-auto bg-white/20 text-white text-xs font-bold px-2 py-0.5 rounded-full">+{inboundQueue.length - 1} more</span>
                    )}
                  </div>
                  <p className="text-white font-black text-xl leading-tight">{call.name}</p>
                  <p className="text-orange-200 text-sm font-mono mt-0.5">{call.number}</p>
                </div>
                ...
                <div className="px-4 py-4 flex gap-3">
                  <button
                    ...
                    className="ring-btn flex-1 bg-orange-500 hover:bg-orange-600 text-white font-black text-sm py-3 rounded-2xl transition-colors flex items-center justify-center gap-2"
                  >
                    <span>📞</span> Answer
                  </button>
                  <button
                    onClick={() => rejectCall(call)}
                    className="flex-1 bg-red-500 hover:bg-red-600 text-white font-black text-sm py-3 rounded-2xl transition-colors flex items-center justify-center gap-2"
                  >
                    <span>📵</span> Decline
                  </button>
                </div>

                {/* Assign to executive (Team Lead only) */}
                {isTeamLead && (
                  <div className="px-4 pb-4">
                    <p className="text-xs text-gray-400 font-semibold mb-2">Or assign to:</p>
                    <div className="flex flex-wrap gap-1.5">
                      {executives.filter(e => e.status === "Available").slice(0, 4).map(e => (
                        <button
                          key={e.id}
                          onClick={() => {
                            const assigned = { ...call, assignedTo: e.name, assignedId: e.id };
                            setInboundQueue(q => q.filter(c => c.id !== call.id));
                            setCallLog(prev => [{
                              ...assigned, outcome: "Assigned", duration: 0, note: `Assigned to ${e.name}`,
                              startedAt: call.arrivedAt, endedAt: new Date().toISOString(),
                            }, ...prev]);
                          }}
                          className="text-xs bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold px-2.5 py-1.5 rounded-lg transition-colors border border-indigo-200"
                        >{e.name.split(" ")[0]}</button>
                      ))}
                      {executives.filter(e => e.status === "Available").length === 0 && (
                        <p className="text-xs text-gray-400 italic">No executives available</p>
                      )}
                    </div>
                  </div>
                )}
        ...
        {/* Team Lead banner */}
        {isTeamLead && (
          <div style={{ background: "linear-gradient(90deg, #312e81, #4338ca, #6366f1)" }} className="px-8 py-2.5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-7 h-7 rounded-full bg-white bg-opacity-20 flex items-center justify-center text-white font-black text-xs">
                {getInitials(execSession.name)}
              </div>
              <div>
                <span className="text-white font-black text-sm">{execSession.name}</span>
                <span className="ml-2 text-indigo-200 text-xs font-semibold uppercase tracking-widest">· Team Lead View</span>
              </div>
            </div>
            <button
              onClick={() => {
              ...
              className="text-indigo-200 hover:text-white text-xs font-bold flex items-center gap-1 transition-colors"
            >
              Sign out ✕
            </button>
          </div>
        ...
        <div className="bg-white border-b border-gray-200 px-8">
          <div className="flex gap-0">
            {TABS.map(tab => {
              const pendingChats = tab === "💬 Chats" && isExec
                ? Object.values(chatSessions).filter(s => s.status === "waiting").length
                : 0;
              return (
                <button
                  key={tab}
                  ...
                  className={`px-4 py-3 text-sm font-semibold border-b-2 transition-colors flex items-center gap-1.5 ${
                    activeTab === tab
                      ? isTeamLead ? "border-indigo-600 text-indigo-700" : "border-orange-600 text-orange-700"
                      : "border-transparent text-gray-500 hover:text-gray-800"
                  }`}
                >
                  {tab}
                  {pendingChats > 0 && (
                    <span className="bg-red-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full leading-none animate-pulse">{pendingChats}</span>
                  )}
                </button>
              );
            })}
        ...
        <div className="flex-1 p-6">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                {(() => {
                  ...
                  const onCall  = Object.keys(execActiveCalls).length;
                    ...
                    >
                      ...
                      {onCall > 0 && (
                        <div className="flex items-center gap-1.5 mt-1.5">
                          ...
                          <p className="text-xs font-semibold text-red-500">{onCall} on call · Busy</p>
                        </div>
                      )}
                    </div>
                  );
          ...
          {/* ═══════════════ LIVE CHAT ═══════════════ */}
          {activeTab === "💬 Live Chat" && (() => {
            const mySession = myChatId ? chatSessions[myChatId] : null;

            /* ── no session yet: start form ── */
            if (!mySession) return (
              <div className="max-w-md mx-auto">
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                  <div style={{ background: "linear-gradient(135deg,#16a34a,#059669)" }} className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-xl">💬</div>
                      <div>
                        <p className="text-white font-black text-base">Live Chat Support</p>
                        <p className="text-orange-100 text-xs">Usually responds in under 2 minutes</p>
                      </div>
                    </div>
                  </div>
                  <div className="p-6 space-y-4">
                    <div>
                      <label className="block text-xs font-bold text-gray-600 mb-1 uppercase tracking-wide">Your Name</label>
                      <input
                        type="text" placeholder={customerSession?.name || user.name || "Enter your name"}
                        value={chatForm.name}
                        onChange={e => setChatForm(f => ({ ...f, name: e.target.value }))}
                        className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-orange-500 transition-colors"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-600 mb-1 uppercase tracking-wide">Phone / Email</label>
                      <input
                        type="text" placeholder={customerSession?.mobile || "+91 XXXXX XXXXX"}
                        value={chatForm.phone}
                        onChange={e => setChatForm(f => ({ ...f, phone: e.target.value }))}
                        className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-orange-500 transition-colors"
                      />
                    </div>
                    <button
                    ...
                    >💬 Start Chat</button>
                    <p className="text-center text-xs text-gray-400">Our support team is available Mon–Sat 9 AM–6 PM</p>
                  </div>
                </div>
              </div>
            );

            const msgs = mySession.messages || [];
            const fmtTs = ts => {
              const d = new Date(ts);
            ...
            /* ── waiting for exec ── */
            if (mySession.status === "waiting") return (
              <div className="max-w-md mx-auto">
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                  <div style={{ background: "linear-gradient(135deg,#16a34a,#059669)" }} className="px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-base">💬</div>
                      <div>
                        <p className="text-white font-bold text-sm">{mySession.customerName}</p>
                        <p className="text-orange-100 text-xs">Connecting to support…</p>
                      </div>
                    </div>
                    <button onClick={endMyChat} className="text-white/60 hover:text-white text-xs font-semibold">✕ Cancel</button>
                  </div>
                  <div className="p-8 flex flex-col items-center gap-4">
                    <div className="flex gap-1.5">
                      {[0,1,2].map(i => (
                        <span key={i} style={{ animationDelay: `${i * 0.2}s` }}
                          className="w-3 h-3 rounded-full bg-orange-400 animate-bounce inline-block" />
                      ))}
                    </div>
                    <p className="text-sm font-semibold text-gray-600">Waiting for an executive…</p>
                    <p className="text-xs text-gray-400">An agent will connect shortly. Please hold on.</p>
                  </div>
                </div>
              </div>
            );
            ...
            /* ── closed ── */
            if (mySession.status === "closed") return (
              <div className="max-w-md mx-auto">
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 text-center space-y-4">
                  <div className="text-4xl">✅</div>
                  <p className="text-base font-black text-gray-800">Chat Ended</p>
                  <p className="text-sm text-gray-500">Thank you for contacting us. We hope we could help!</p>
                  <button
                    onClick={() => { sessionStorage.removeItem("myChatId"); setMyChatId(null); setChatForm({ name: "", phone: "" }); }}
                    className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-2.5 rounded-xl transition-colors"
            ...
            /* ── active chat ── */
            return (
              <div className="max-w-lg mx-auto flex flex-col" style={{ height: "72vh" }}>
                {/* Header */}
                <div style={{ background: "linear-gradient(135deg,#16a34a,#059669)" }} className="px-5 py-3 rounded-t-2xl flex items-center justify-between flex-shrink-0">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center font-bold text-white text-xs">
                      {getInitials(mySession.execName || "S")}
                    </div>
                    <div>
                      <p className="text-white font-bold text-sm">{mySession.execName || "Support"}</p>
                  ...
                  <button onClick={endMyChat} className="text-white/70 hover:text-white text-xs font-semibold px-3 py-1 rounded-lg bg-white/10 hover:bg-white/20 transition-colors">End Chat</button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto bg-gray-50 p-4 space-y-3 border-x border-gray-200">
                  {msgs.length === 0 && (
                    <div className="text-center py-8">
                      <p className="text-2xl mb-2">👋</p>
                      <p className="text-sm text-gray-400 font-semibold">Say hello to start the conversation</p>
                    </div>
                  )}
                  {msgs.map(m => {
                    ...
                    return (
                      <div key={m.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[78%] px-4 py-2.5 rounded-2xl text-sm shadow-sm ${
                          isMe ? "bg-orange-600 text-white rounded-br-sm" : "bg-white text-gray-800 border border-gray-200 rounded-bl-sm"
                        }`}>
                          <p>{m.text}</p>
                          <p className={`text-xs mt-1 ${isMe ? "text-orange-200" : "text-gray-400"}`}>{fmtTs(m.ts)}</p>
                        </div>
                      </div>
                    );
                  })}
                ...
                {/* Input */}
                <div className="border-x border-b border-gray-200 rounded-b-2xl bg-white p-3 flex gap-2 flex-shrink-0">
                  <input
                    type="text"
                    value={chatInput}
                    ...
                    className="flex-1 border-2 border-gray-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-orange-500 transition-colors"
                  />
                  <button
                    onClick={sendChatMsg}
                    disabled={!chatInput.trim()}
                    className="bg-orange-600 hover:bg-orange-700 disabled:opacity-40 text-white font-bold px-4 py-2 rounded-xl transition-colors flex items-center gap-1"
                  >➤</button>
                </div>
              </div>
            );
          ...
          {activeTab === "💬 Chats" && (() => {
            ...
            const activeList   = allSessions.filter(s => s.execId === execSession?.id && s.status === "active");
            const closedList   = allSessions.filter(s => s.execId === execSession?.id && s.status === "closed");
            const currentChat  = execChatActiveId ? chatSessions[execChatActiveId] : null;
            const fmtTs = ts => new Date(ts).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });

            return (
              <div className="flex gap-5" style={{ height: "72vh" }}>
                ...
                <div className="w-72 flex-shrink-0 flex flex-col gap-3 overflow-y-auto pr-1">
                      ...
                      {waitingList.map(s => (
                        <div key={s.id} className="bg-white rounded-xl border border-amber-200 p-3">
                          <div className="flex items-center justify-between mb-2">
                            <div>
                              <p className="text-sm font-bold text-gray-800">{s.customerName}</p>
                              <p className="text-xs text-gray-400">{s.customerPhone}</p>
                            </div>
                            <span className="text-xs text-amber-600 font-semibold">{Math.floor((Date.now() - s.startedAt) / 60000)}m</span>
                          </div>
                          <button onClick={() => acceptChatAsExec(s.id)}
                  ...
                  {activeList.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs font-black text-gray-500 uppercase tracking-wide px-1">💬 My Active</p>
                      {activeList.map(s => {
                        const last = s.messages[s.messages.length - 1];
                        return (
                          <button key={s.id} onClick={() => setExecChatActiveId(s.id)}
                            className={`w-full text-left rounded-xl border-2 p-3 transition-all ${execChatActiveId === s.id ? "border-orange-500 bg-orange-50" : "border-gray-200 bg-white hover:border-orange-300"}`}
                          >
                            <p className="text-sm font-bold text-gray-800">{s.customerName}</p>
                            <p className="text-xs text-gray-400 truncate">{last ? last.text : "No messages"}</p>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  {closedList.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs font-black text-gray-400 uppercase tracking-wide px-1">✓ Closed</p>
                      {closedList.slice(0, 5).map(s => (
                        <button key={s.id} onClick={() => setExecChatActiveId(s.id)}
                          className={`w-full text-left rounded-xl border-2 p-3 opacity-60 ${execChatActiveId === s.id ? "border-gray-400 bg-gray-50" : "border-gray-200 bg-white"}`}
                        >
                          <p className="text-sm font-bold text-gray-700">{s.customerName}</p>
                          <p className="text-xs text-gray-400">{s.messages.length} messages</p>
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  {waitingList.length === 0 && activeList.length === 0 && closedList.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-16 text-center gap-3">
                      ...
                      <p className="text-sm font-semibold text-gray-400">No chats yet</p>
                      <p className="text-xs text-gray-300">Customers will appear when they start a chat</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex-1 flex flex-col min-w-0">
                  {!currentChat ? (
                    <div className="flex-1 bg-white rounded-2xl border-2 border-dashed border-gray-200 flex items-center justify-center">
                      <div className="text-center">
                        <p className="text-3xl mb-2">💬</p>
                        <p className="text-sm font-semibold text-gray-400">Select a chat to open</p>
                      </div>
                    </div>
                  ) : (
                    <div className="flex-1 flex flex-col bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                      <div className="px-5 py-3 border-b border-gray-100 bg-gray-50 flex items-center justify-between flex-shrink-0">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-bold text-gray-800">{currentChat.customerName}</p>
                            <p className="text-xs text-gray-400">{currentChat.customerPhone} · {currentChat.status === "active" ? "🟢 Active" : "⛔ Closed"}</p>
                          </div>
                        </div>
                        {currentChat.status === "active" && (
                          <button onClick={() => closeExecChatSession(execChatActiveId)}
                            className="text-xs bg-red-50 hover:bg-red-100 text-red-600 font-bold px-3 py-1.5 rounded-lg transition-colors"
                          >End Chat</button>
                        )}
                      </div>
                      ...
                      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
                        ...
                        {currentChat.messages.map(m => {
                          const isMe = m.sender === "exec";
                          return (
                            <div key={m.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                              <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm shadow-sm ${
                                isMe ? "bg-orange-600 text-white rounded-br-sm" : "bg-white text-gray-800 border border-gray-200 rounded-bl-sm"
                              }`}>
                                {!isMe && <p className="text-xs font-bold text-orange-600 mb-1">{m.senderName}</p>}
                                <p>{m.text}</p>
                                <p className={`text-xs mt-1 ${isMe ? "text-orange-200" : "text-gray-400"}`}>{fmtTs(m.ts)}</p>
                              </div>
                            </div>
                      ...
                      {currentChat.status === "active" ? (
                        <div className="border-t border-gray-100 p-3 flex gap-2 flex-shrink-0">
                          <input type="text" value={execChatReply}
                            onChange={e => setExecChatReply(e.target.value)}
                            onKeyDown={e => e.key === "Enter" && sendExecChatMsg()}
                            placeholder="Type a reply…"
                            className="flex-1 border-2 border-gray-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-orange-500 transition-colors"
                          />
                          <button onClick={sendExecChatMsg} disabled={!execChatReply.trim()}
                            className="bg-orange-600 hover:bg-orange-700 disabled:opacity-40 text-white font-bold px-4 py-2 rounded-xl transition-colors"
                          >➤</button>
                        </div>
                      ) : (
                        <div className="border-t border-gray-100 p-3 text-center text-xs text-gray-400 font-semibold">
                          This chat has ended
                        </div>
                      )}
                    </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                {[
                  ...
                  { label: "Busy",               value: executivesWithCounts.filter(e => e.status === "Busy" || execActiveCalls[e.id]).length,    color: "text-amber-600",   bg: "bg-amber-50" },
                  { label: "On Call",            value: Object.keys(execActiveCalls).length,                                                      color: "text-red-600",     bg: "bg-red-50" },
              ...
              {/* ── INCENTIVE POLICY ── */}
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-5 py-3 bg-gradient-to-r from-indigo-50 to-fuchsia-50 border-b border-gray-100">
                  <div className="flex items-center gap-3">
                    <span className="text-base">🎁</span>
                    <p className="text-xs font-bold text-gray-700 uppercase tracking-widest">Incentive Policy</p>
                    {(() => {
                      const on = (incentiveDraft || incentivePolicy).enabled !== false;
                      return (
                        <button
                          onClick={toggleIncentiveEnabled}
                          role="switch" aria-checked={on}
                          title={on ? "Incentives enabled — click to disable" : "Incentives disabled — click to enable"}
                          style={{ color: on ? "#16a34a" : "#dc2626" }}
                          className={`relative inline-flex items-center gap-2 pl-2 pr-2.5 py-1 rounded-full text-xs font-bold transition-colors ${on ? "bg-green-100" : "bg-red-100"}`}
                        >
                          <span className={`w-8 h-4 rounded-full relative transition-colors ${on ? "bg-green-500" : "bg-red-500"}`}>
                  ...
                  <div className="flex items-center gap-2">
                    {incentiveSaved && <span className="text-xs font-bold text-emerald-600">✓ Saved</span>}
                    {incentiveDraft ? (
                      <>
                        <button
                          onClick={() => saveIncentivePolicy(incentiveDraft)}
                          className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-3 py-1.5 rounded-lg transition-colors"
                        >Save Policy</button>
                        <button
                          onClick={() => setIncentiveDraft(null)}
                          className="text-gray-400 hover:text-gray-600 text-xs font-bold px-2 py-1.5"
                        >Cancel</button>
                      </>
                    ) : (
                      <>
                        <button
                          onClick={() => setIncentiveDraft(JSON.parse(JSON.stringify(incentivePolicy)))}
                          className="text-xs text-indigo-500 hover:text-indigo-700 font-semibold border border-indigo-200 px-3 py-1.5 rounded-lg hover:bg-indigo-50 transition-colors"
                        >✎ Edit</button>
                        <button
                          onClick={() => { if (window.confirm("Reset incentive policy to defaults?")) saveIncentivePolicy(DEFAULT_INCENTIVE_POLICY); }}
                          className="text-xs text-gray-400 hover:text-red-600 border border-gray-200 px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-colors"
                        >↺ Defaults</button>
                      </>
                    )}
                  </div>
                ...
                {(incentiveDraft || incentivePolicy).enabled === false && (
                  <div className="px-5 py-2.5 bg-amber-50 border-b border-amber-100 text-xs font-semibold text-amber-700">
                    ⚠ Incentives are turned off — executives won't see the Incentives tab. Rates below still save for when you re-enable it.
                  </div>
                )}

                {(() => {
                  const p = incentiveDraft || incentivePolicy;
                  const editing = !!incentiveDraft;
                  const cur = p.currency || "₹";
                  const setField = (key, val) => setIncentiveDraft(d => ({ ...d, [key]: val }));
                  const setTier = (i, key, val) => setIncentiveDraft(d => ({
                    ...d, tiers: d.tiers.map((t, idx) => idx === i ? { ...t, [key]: val } : t),
                  }));
                  const numField = (label, key, prefix = cur) => (
                      ...
                      {editing ? (
                        <div className="flex items-center gap-1">
                          ...
                          <input
                            ...
                            className="w-20 border border-indigo-300 rounded-lg px-2 py-1 text-sm font-bold text-right focus:outline-none focus:border-indigo-500"
                          />
                        </div>
                      ) : (
                        <span className="text-sm font-bold text-gray-800">{prefix}{p[key]}</span>
                      )}
                    </div>
                  );
                  return (
                    <div className="p-5 grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2.5">
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-1">Rates & Bonuses</p>
                        {numField("Per resolved ticket", "perResolvedTicket")}
                        {numField("Per answered call", "perAnsweredCall")}
                        {numField("Per chat handled", "perChatHandled")}
                        {numField("Answer-rate target", "answerRateTarget", "")}
                        {numField("Answer-rate bonus", "answerRateBonus")}
                        {numField("Mandatory-hours bonus", "hoursMetBonus")}
                      </div>

                      {/* Tiers */}
                      <div className="space-y-2">
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-1">Reward Tiers</p>
                        {p.tiers.map((t, i) => (
                            ...
                            {editing ? (
                              <>
                                <input
                                  ...
                                  className="w-24 border border-gray-200 rounded-lg px-2 py-1 text-sm font-bold focus:outline-none focus:border-indigo-500"
                                />
                                <div className="flex items-center gap-1">
                                  <span className="text-xs text-gray-400">from {cur}</span>
                                  <input
                                    type="number" min="0"
                                    value={t.min}
                                    onChange={e => setTier(i, "min", e.target.value === "" ? "" : Number(e.target.value))}
                                    className="w-20 border border-gray-200 rounded-lg px-2 py-1 text-sm text-right focus:outline-none focus:border-indigo-500"
                                  />
                                </div>
                                <div className="flex items-center gap-1 ml-auto">
                                  ...
                                  <input
                                    type="number" min="0"
                                    value={t.bonus}
                                    onChange={e => setTier(i, "bonus", e.target.value === "" ? "" : Number(e.target.value))}
                                    className="w-20 border border-gray-200 rounded-lg px-2 py-1 text-sm text-right font-bold text-emerald-600 focus:outline-none focus:border-indigo-500"
                                  />
                                </div>
                              </>
                            ) : (
                              <>
                                <span className="text-sm font-bold text-gray-800 w-24">{t.label}</span>
                                <span className="text-xs text-gray-400">from {cur}{Number(t.min).toLocaleString("en-IN")}</span>
                                <span className="ml-auto text-sm font-bold text-emerald-600">{t.bonus > 0 ? `+${cur}${t.bonus}` : "—"}</span>
                              </>
                            )}
                          </div>
                        ))}
                        <p className="text-xs text-gray-400 pt-1">
                          Changes apply live to every executive's Incentives tab.
                        </p>
                      </div>
                    </div>
                  );
              ...
              {execView === "cards" && <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="lg:col-span-1 space-y-2">
                  {executivesWithCounts.map(exec => {
                    ...
                    const isOnCall = !!execActiveCalls[exec.id];
                    return (
                      <div
                        key={exec.id}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 relative ${isOnCall ? "bg-red-100 text-red-700" : "bg-orange-100 text-orange-700"}`}>
                            {getInitials(exec.name)}
                            {isOnCall && <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full flex items-center justify-center text-white text-xs leading-none">📞</span>}
                          </div>
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              <p className="text-sm font-bold text-gray-800 truncate">{exec.name}</p>
                              <span className={`text-xs px-1.5 py-0.5 rounded-full font-semibold flex-shrink-0 ${
                                isOnCall            ? "bg-red-100 text-red-700"
                                : exec.status === "Available" ? "bg-emerald-100 text-emerald-700"
                                : exec.status === "Busy"      ? "bg-amber-100 text-amber-700"
                                :                               "bg-gray-100 text-gray-500"
                              }`}>{isOnCall ? "📞 On Call" : exec.status}</span>
                            </div>
                            <p className="text-xs text-gray-400">{exec.role || "Agent"}</p>
                          </div>
                        </div>
                        ...
                        {isOnCall && (
                          <button
                            onClick={e => {
                              e.stopPropagation();
                              setBargeInSession({ ...execActiveCalls[exec.id], userName: execActiveCalls[exec.id].name });
                              setBargeMode("listen");
                            }}
                            className="mt-3 w-full bg-red-600 hover:bg-red-700 text-white text-xs font-bold py-1.5 rounded-lg transition-colors flex items-center justify-center gap-1.5"
                          >
                            🎧 Barge In
                          </button>
                        )}
                          ...
                          {inboundQueue.map((call) => (
                            <div key={call.id} className="flex items-center gap-3 px-5 py-3.5 hover:bg-gray-50 transition-colors">
                              ...
                              <div className="flex gap-2 flex-shrink-0">
                                ...
                                {isTeamLead && (
                                  <button
                                    onClick={() => setTransferTarget({ call, source: "queue" })}
                                    className="bg-indigo-100 hover:bg-indigo-200 text-indigo-700 text-xs font-bold px-3 py-1.5 rounded-lg transition-all"
                                  >↗ Assign</button>
                                )}
                                <button
                      ...
                      <div className="bg-white rounded-2xl border-2 border-orange-200 shadow-lg overflow-hidden">
                        ...
                        <div className="p-5 space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            <button
                              onClick={() => setTransferTarget({ call: activeCall, source: "active" })}
          ...
          {activeTab === "👥 Users" && (() => {
            ...
            tickets.forEach(t => {
              ...
              if (!key) return;
              if (!userMap[key]) {
                userMap[key] = { key, name: t.name || "Unknown", email: t.email || "", phone: t.phone || "", tickets: [] };
              }
              userMap[key].tickets.push(t);
            });
            const allUsers = Object.values(userMap);
            const q = userSearch.toLowerCase();
            const filtered = q
              ? allUsers.filter(u => u.name.toLowerCase().includes(q) || u.email.toLowerCase().includes(q) || u.phone.includes(q))
              : allUsers;
            const isBlocked = key => blockedUsers.includes(key);
            const activeCalls = Object.values(execActiveCalls);
            const getActiveCall = (u) => activeCalls.find(c =>
              u.phone && c.number && (
                c.number.replace(/\s+/g, "") === u.phone.replace(/\s+/g, "") ||
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{allUsers.length} unique customer{allUsers.length !== 1 ? "s" : ""} · {blockedUsers.length} blocked</p>
                  </div>
                  <input
                    type="text"
                    ...
                    className="border-2 border-gray-200 rounded-xl px-3.5 py-2 text-sm w-64 focus:outline-none focus:border-indigo-400 transition-colors"
                  />
                </div>
                ...
                <div className="flex gap-5">
                  <div className="flex-1 space-y-3">
                    {filtered.length === 0 && (
                      <div className="bg-white rounded-xl border border-gray-200 p-10 text-center text-gray-400 text-sm">No users found</div>
                    )}
                    {filtered.map(u => {
                      const open     = u.tickets.filter(t => t.status === "Open").length;
                      const resolved = u.tickets.filter(t => t.status === "Resolved" || t.status === "Closed").length;
                      const blocked  = isBlocked(u.key);
                      const active   = selectedUser?.key === u.key;
                      const liveCall = getActiveCall(u);
                      return (
                        <div
                          key={u.key}
                          onClick={() => setSelectedUser(active ? null : u)}
                          className={`bg-white rounded-xl border-2 p-4 cursor-pointer transition-all ${
                            active ? "border-indigo-400 shadow-md" : blocked ? "border-red-200 opacity-70" : "border-gray-100 hover:border-indigo-200 hover:shadow-sm"
                          }`}
                        >
                          <div className="flex items-center gap-4">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-sm flex-shrink-0 ${blocked ? "bg-red-100 text-red-600" : "bg-indigo-100 text-indigo-700"}`}>
                              {getInitials(u.name)}
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <p className="font-bold text-gray-900 text-sm truncate">{u.name}</p>
                                {blocked && <span className="text-xs bg-red-100 text-red-600 font-bold px-2 py-0.5 rounded-full">Blocked</span>}
                              </div>
                              <p className="text-xs text-gray-400 truncate">{u.email || u.phone}</p>
                            </div>
                            <div className="flex items-center gap-2 flex-shrink-0 text-xs">
                              {liveCall && <span className="flex items-center gap-1 bg-orange-100 text-orange-700 font-bold px-2 py-1 rounded-lg animate-pulse">🔴 Live</span>}
                              <span className="bg-blue-50 text-blue-600 font-bold px-2 py-1 rounded-lg">{u.tickets.length} tickets</span>
                              {open > 0 && <span className="bg-amber-50 text-amber-600 font-bold px-2 py-1 rounded-lg">{open} open</span>}
                              {resolved > 0 && <span className="bg-emerald-50 text-emerald-600 font-bold px-2 py-1 rounded-lg">{resolved} resolved</span>}
                            </div>
                            <div className="flex gap-2 flex-shrink-0">
                              {liveCall && (
                                <button onClick={e => { e.stopPropagation(); setBargeInSession({ ...liveCall, userName: u.name }); setBargeMode("listen"); }}
                                  className="text-xs bg-orange-600 hover:bg-orange-700 text-white font-bold px-3 py-1.5 rounded-lg transition-colors">🎧 Barge In</button>
                              )}
                              {blocked ? (
                                <button onClick={e => { e.stopPropagation(); const upd = blockedUsers.filter(k => k !== u.key); setBlockedUsers(upd); localStorage.setItem("blockedUsers", JSON.stringify(upd)); }}
                                  className="text-xs bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold px-3 py-1.5 rounded-lg transition-colors">Unblock</button>
                              ) : (
                                <button onClick={e => { e.stopPropagation(); setBlockModal(u); setBlockReason(""); }}
                                  className="text-xs bg-red-50 hover:bg-red-100 text-red-600 font-bold px-3 py-1.5 rounded-lg transition-colors">Block</button>
                              )}
                            </div>
                          </div>
                        </div>
                  ...
                  {selectedUser && (
                    <div className="w-80 flex-shrink-0 bg-white rounded-xl border-2 border-indigo-200 shadow-sm self-start sticky top-24">
                      <div style={{ background: "linear-gradient(135deg,#312e81,#4338ca)" }} className="rounded-t-xl p-5 text-white">
                        <div className="flex items-center justify-between mb-3">
                          <p className="text-xs font-bold uppercase tracking-widest text-indigo-200">User Profile</p>
                          <button onClick={() => setSelectedUser(null)} className="text-indigo-200 hover:text-white font-bold">✕</button>
                        </div>
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-full bg-white bg-opacity-20 flex items-center justify-center font-black text-lg">{getInitials(selectedUser.name)}</div>
                          <div>
                            <p className="font-black text-base">{selectedUser.name}</p>
                            {isBlocked(selectedUser.key) && <span className="text-xs bg-red-500 text-white font-bold px-2 py-0.5 rounded-full">Blocked</span>}
                          </div>
                        </div>
                      </div>
                      <div className="p-5 space-y-4">
                        <div className="space-y-2 text-sm">
                          {selectedUser.email && <div className="flex items-center gap-2 text-gray-600"><span>📧</span><span className="truncate">{selectedUser.email}</span></div>}
                          {selectedUser.phone && <div className="flex items-center gap-2 text-gray-600"><span>📱</span><span>{selectedUser.phone}</span></div>}
                        </div>
                        <div className="grid grid-cols-3 gap-2 text-center">
                          {[
                            { label: "Total",    val: selectedUser.tickets.length, color: "text-indigo-600" },
                            { label: "Open",     val: selectedUser.tickets.filter(t => t.status === "Open" || t.status === "In Progress").length, color: "text-amber-600" },
                            { label: "Resolved", val: selectedUser.tickets.filter(t => t.status === "Resolved" || t.status === "Closed").length, color: "text-emerald-600" },
                          ].map(({ label, val, color }) => (
                        ...
                        <div>
                          <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Ticket History</p>
                          <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                            {selectedUser.tickets.length === 0 && <p className="text-xs text-gray-400">No tickets</p>}
                            {[...selectedUser.tickets].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map(t => (
                              <div key={t.id} className="border border-gray-100 rounded-lg p-2.5 text-xs">
                                <div className="flex items-center justify-between mb-0.5">
                                  <span className="font-bold text-gray-700 truncate flex-1 mr-2">#{t.id?.slice(-4)} {t.subject}</span>
                                  <span className={`font-bold flex-shrink-0 ${t.status === "Resolved" || t.status === "Closed" ? "text-emerald-600" : t.status === "In Progress" ? "text-amber-600" : "text-blue-600"}`}>{t.status}</span>
                                </div>
                                <p className="text-gray-400">{t.category} · {t.priority}</p>
                                <p className="text-gray-400">Assigned: {t.assignedTo || "Unassigned"}</p>
                              </div>
                            ))}
                          </div>
                        </div>
                        {(() => {
                          const liveCall = getActiveCall(selectedUser);
                          return liveCall ? (
                            <div className="bg-orange-50 border border-orange-200 rounded-xl p-3">
                              <p className="text-xs font-bold text-orange-700 mb-1">🔴 Live Call in Progress</p>
                              <p className="text-xs text-orange-600">With: {liveCall.execName}</p>
                              <p className="text-xs text-orange-600">{liveCall.direction === "inbound" ? "📥 Inbound" : "📤 Outbound"}</p>
                              <button onClick={() => { setBargeInSession({ ...liveCall, userName: selectedUser.name }); setBargeMode("listen"); }}
                                className="mt-2 w-full bg-orange-600 hover:bg-orange-700 text-white font-bold text-xs py-2 rounded-xl transition-colors">🎧 Barge In</button>
                            </div>
                          ) : null;
                        })()}
                        <div className="flex gap-2 pt-2 border-t border-gray-100">
                          {isBlocked(selectedUser.key) ? (
                            <button onClick={() => { const upd = blockedUsers.filter(k => k !== selectedUser.key); setBlockedUsers(upd); localStorage.setItem("blockedUsers", JSON.stringify(upd)); }}
                              className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold text-xs py-2 rounded-xl transition-colors">Unblock User</button>
                          ) : (
                            <button onClick={() => { setBlockModal(selectedUser); setBlockReason(""); }}
                              className="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold text-xs py-2 rounded-xl transition-colors">Block User</button>
                          )}
                        </div>
                      </div>
                    </div>
      ...
      {/* ── Transfer / Assign call to executive ── */}
      {transferTarget && (() => {
        const now = Date.now();
        const isOnline = (id) => (now - (onlineSessions[id] || 0)) < 120000;
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 80 }}
            ...
            <div onClick={e => e.stopPropagation()}
              ...
              <div className="px-5 py-4 bg-gradient-to-r from-indigo-600 to-violet-600 text-white flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest text-indigo-100">
                    {transferTarget.source === "active" ? "Transfer Call" : "Assign Call"}
                  </p>
                  <p className="text-sm font-bold mt-0.5">{transferTarget.call.name} · {transferTarget.call.number}</p>
                </div>
                <button onClick={() => setTransferTarget(null)} className="text-white/80 hover:text-white text-lg">✕</button>
              </div>
              {/* Executive list */}
              <div className="max-h-80 overflow-y-auto divide-y divide-gray-50">
                {executivesWithCounts.filter(e => e.id !== execSession?.id).length === 0 ? (
                  <div className="px-5 py-8 text-center text-sm text-gray-400">No executives available</div>
                ) : (
                  executivesWithCounts
                    .filter(e => e.id !== execSession?.id)
                    .sort((a, b) => (isOnline(b.id) ? 1 : 0) - (isOnline(a.id) ? 1 : 0))
                    .map(exec => {
                      const online = isOnline(exec.id);
                      const onCall = !!execActiveCalls[exec.id];
                      return (
                        <div key={exec.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition-colors">
                          <div className="w-9 h-9 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold text-xs flex-shrink-0">
                            {getInitials(exec.name)}
                          </div>
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs text-gray-400">
                              {exec.role || "Agent"} ·{" "}
                              <span className={online ? "text-emerald-600 font-semibold" : "text-gray-400"}>
                                {online ? "● Online" : "○ Offline"}
                              </span>
                              {onCall && <span className="text-amber-600 font-semibold"> · On call</span>}
                            </p>
                          </div>
                          <button
                            onClick={() => assignCallToExec(transferTarget.call, exec)}
                            disabled={onCall}
                            className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs font-bold px-3.5 py-1.5 rounded-lg transition-colors flex-shrink-0"
                          >{onCall ? "Busy" : "Assign"}</button>
                        </div>
                      );
                    })
                )}
              ...
              <div className="px-5 py-2.5 bg-gray-50 border-t border-gray-100 text-xs text-gray-400">
                Offline executives will receive the call when they next open the portal.
              </div>
            </div>
          </div>
        );
      ...
      {bargeInSession && (
        <>
          <style>{`
            @keyframes bargePulse { 0%,100%{opacity:1} 50%{opacity:0.4} }
            .barge-dot { animation: bargePulse 1.2s infinite; }
          `}</style>
          ...
          <div style={{
            ...
            {/* Header */}
            <div style={{ background: "linear-gradient(135deg, #064e3b, #065f46, #047857)" }} className="p-6 text-white">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <span className="barge-dot w-2.5 h-2.5 rounded-full bg-orange-400 inline-block" />
                  <span className="text-xs font-bold uppercase tracking-widest text-orange-200">Barge In Active</span>
                </div>
                <span className="font-mono text-orange-100 text-sm font-bold">{fmtDuration(bargeTimer)}</span>
              </div>
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-white/20 flex items-center justify-center text-2xl font-black">
                  {getInitials(bargeInSession.userName || bargeInSession.name)}
                </div>
                <div>
                  <p className="font-black text-lg">{bargeInSession.userName || bargeInSession.name}</p>
                  <p className="text-orange-200 text-sm">{bargeInSession.number}</p>
                  <p className="text-orange-300 text-xs mt-0.5">
                    {bargeInSession.direction === "inbound" ? "📥 Inbound" : "📤 Outbound"} · with {bargeInSession.execName}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 space-y-5">
              {/* Barge mode selector */}
              <div>
                <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Barge Mode</p>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { id: "listen",  label: "🎧 Listen",  desc: "Silent — hear both sides" },
                    { id: "whisper", label: "🗣 Whisper", desc: "Speak to exec only" },
                    { id: "full",    label: "🔊 Full",     desc: "Join the conversation" },
                  ].map(m => (
                    <button
                      key={m.id}
                      onClick={() => setBargeMode(m.id)}
              ...
              <div className={`rounded-xl p-3 text-sm font-semibold flex items-center gap-2 ${
                bargeMode === "listen"  ? "bg-blue-50 text-blue-700" :
                bargeMode === "whisper" ? "bg-amber-50 text-amber-700" :
                                          "bg-orange-50 text-orange-700"
              }`}>
                {bargeMode === "listen"  && "🎧 You are listening silently. The exec and customer cannot hear you."}
                {bargeMode === "whisper" && "🗣 Whispering to exec only. The customer cannot hear you."}
                {bargeMode === "full"    && "🔊 Full barge — all three parties can hear each other."}
              </div>

              {/* Exec + Customer info */}
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-indigo-50 rounded-xl p-3">
                  <p className="text-xs font-bold text-indigo-500 uppercase mb-1">Executive</p>
                  <p className="font-bold text-gray-800 text-sm">{bargeInSession.execName}</p>
                  <p className="text-xs text-indigo-400">{bargeInSession.execId}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-3">
                  <p className="text-xs font-bold text-gray-400 uppercase mb-1">Customer</p>
                  <p className="font-bold text-gray-800 text-sm">{bargeInSession.userName || bargeInSession.name}</p>
                  <p className="text-xs text-gray-400 font-mono">{bargeInSession.number}</p>
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-1">
                <button
                  onClick={() => setBargeInSession(null)}
                  className="flex-1 border-2 border-gray-200 text-gray-600 font-bold text-sm py-2.5 rounded-xl hover:bg-gray-50 transition-colors"
                >Leave Silently</button>
                <button
                  onClick={() => {
                    /* In a real system this would signal the exec to end. Here we just exit barge. */
                    setBargeInSession(null);
                  }}
      ...
      {blockModal && (
        <>
          <div onClick={() => setBlockModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 60 }} />
          <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 61, width: 420, background: "white", borderRadius: 20, boxShadow: "0 30px 60px rgba(0,0,0,0.25)", padding: 28 }}>
            <p className="text-base font-black text-gray-900 mb-1">Block User</p>
            <p className="text-xs text-gray-500 mb-4">This user will be flagged and their future submissions may be reviewed.</p>

            <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-4 flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-red-200 flex items-center justify-center text-red-700 font-black text-sm flex-shrink-0">
                {getInitials(blockModal.name)}
              </div>
              <div>
                <p className="font-bold text-gray-800 text-sm">{blockModal.name}</p>
                <p className="text-xs text-gray-500">{blockModal.email || blockModal.phone}</p>
              </div>
            </div>
            ...
            <div className="mb-5">
              ...
              <textarea
                rows={3}
                placeholder="e.g. Abusive behaviour, spam submissions…"
                value={blockReason}
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  const updated = [...blockedUsers, blockModal.key];
                  setBlockedUsers(updated);
                  localStorage.setItem("blockedUsers", JSON.stringify(updated));
                  if (selectedUser?.key === blockModal.key) setSelectedUser(s => ({ ...s }));
                  setBlockModal(null);
                  setBlockReason("");
                }}
