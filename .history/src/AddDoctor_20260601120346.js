...
if (newDoctor.availableDays.length === 0) {
  ...
  const handleSave = () => {
    ...
    // ensure availableDays are stored in canonical order
    const daysOrder = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"];
    const orderedDays = (newDoctor.availableDays || []).filter(d => daysOrder.includes(d)).sort((a,b)=> daysOrder.indexOf(a) - daysOrder.indexOf(b));
    const toSave = { ...newDoctor, availableDays: orderedDays };

    const storedDoctors = JSON.parse(localStorage.getItem("doctors")) || [];
    storedDoctors.push(toSave);
