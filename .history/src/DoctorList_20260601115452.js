...
function DoctorList() {
  ...
  // Return a normalized status key used for coloring and logic
  const getAppointmentStatusKey = (appt) => {
    ...
    if (appt?.status) {
      ...
      // allow other custom statuses to pass through
      return s;
    }

    // Check common alternate flags/fields set by booking/payment flows
    if (appt?.paid || appt?.isPaid === true) return "PAID";
    if (appt?.paymentStatus && String(appt.paymentStatus).toUpperCase() === "PAID") return "PAID";
    if (appt?.payment_status && String(appt.payment_status).toUpperCase() === "PAID") return "PAID";
    if (appt?.payment && String(appt.payment).toUpperCase() === "PAID") return "PAID";
    if (appt?.paymentMethod || appt?.payment_method || appt?.paidVia) {
      // presence of a payment method usually implies paid
      return "PAID";
    }

    // Fallback: check global paidAppointments map in localStorage by id
    try {
    ...
    // If no explicit date set, treat as pending
    if (!appt?.date) return "PENDING";

    // Determine by date: past -> completed, future -> pending
  ...
  // Return a human-friendly display label, including payment method when available
  const getAppointmentDisplay = (appt) => {
    const key = getAppointmentStatusKey(appt);
    if (key === "PAID") {
      // Try explicit fields first
      const method =
        appt?.paymentMethod || appt?.payment || appt?.payment_method || appt?.paidVia || appt?.paymentMode || null;

      // fallback to global paidAppointments store by id
      ...
      // If payment info isn't available but appointment has fields like `paid` use a generic label
...
localStorage.setItem(
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      {/* Left Navigation Bar */}
      <div className="w-64 bg-white shadow-lg fixed left-0 top-0 h-full border-r border-gray-200 flex flex-col">
        {/* Logo and Navigation */}
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50">
          <div className="flex items-center gap-3 px-2 py-3 rounded-lg bg-white hover:bg-gray-100 transition duration-200 cursor-pointer shadow-sm">
...
{doc.availableDays?.length > 0 && (
 ...
 {Array.isArray(doc.appointments) && doc.appointments.length > 0 && (
  <div className="w-full mt-2 bg-gradient-to-r from-blue-50 to-indigo-50 border border-indigo-200 rounded-lg overflow-hidden">
    ...
    <div className="p-2 max-h-46 overflow-y-auto">
      {doc.appointments.map((appt, idx) => (
        ...
        >
          ...
          <span
            className={`inline-block mt-1 px-2 py-1 rounded-full text-white text-[10px]
            ${
              getAppointmentStatusKey(appt) === "PAID"
                ? "bg-green-500"
