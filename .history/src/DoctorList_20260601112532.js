...
function DoctorList() {
  ...
  const getAppointmentDisplay = (appt) => {
    ...
    if (key === "PAID") {
      ...
      const method =
        appt?.paymentMethod || appt?.payment || appt?.payment_method || appt?.paidVia || null;
  ...
  <div className="w-full">
    ...
    <div className="flex flex-wrap gap-1">
      {doc.availableDays.map((day, i) => (
        <span
          key={i}
          className="px-2 py-1 text-[10px] bg-green-100 text-black-700 rounded-full border border-green-300"
        >
          {day.slice(0, 3)}
        </span>
      ))}
          ...
          <span
            ...
            ${
              getAppointmentStatus(appt) === "PAID"
                ...
                ? "bg-blue-500"
                : getAppointmentStatus(appt) === "CANCELLED"
                ? "bg-red-500"
                : "bg-orange-500"
            }`}
          >
